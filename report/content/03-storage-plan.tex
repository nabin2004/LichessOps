...
%     \item Predicate push-down with Spark/MariaDB (partition pruning on year/month).
% \end{itemize}

% \subsection{Why MinIO?}

\subsection{Logical structure (Star Schema)}
At this stage of data ingestion, processed data is modelled as a star schema centred on \texttt{games}. The design is shown in \cref{fig:dataset-one-logical-erd} as a crow-foot ERD. Justification for the star schema is given in depth in section \ref{sec:justi_star_schema}
