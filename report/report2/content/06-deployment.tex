...
A minimal check I repeat after each promote: \texttt{GET /health} returns loaded=true, then \texttt{POST /predict} with a 1800-rated blitz game and the Sicilian Defence headers I used in tests. If probabilities sum to roughly one and the model gauge flips to 1, the deploy is good enough for demo purposes.
