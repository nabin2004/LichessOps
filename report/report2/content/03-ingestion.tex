...
I chose MinIO because PGN and \texttt{.zst} blobs do not map cleanly onto a columnar warehouse table. Object storage treats them as opaque files with cheap immutability and no semantics. This enables the reprocessing of any month's data later without re-downloading it. Here, I also do the checksum validation for assesing the reliability and authenticity of data. Catching these issues early helps to save time and resources early in the pipeline. Airflow orchestration runs the DAG on each month on first day, so developer does not need to manually click download when Lichess typically publishes. ELT fits my development setup. Here transformation stays on Spark and MariaDB.
...
%   \item Downstream tasks:  (\texttt{spark\_transform}, \texttt{MariaDB\_sync}) consume the raw key which they are preprocessing but depend on this stage finishing cleanly.
% \end{enumerate}
...
For retrieval, \texttt{raw\_s3\_uri(month)} returns the full \texttt{s3://} key, which MariaDB's \texttt{httpfs} extension or Spark's S3 configuration consumes. That indirection is how ingestion meets the brief's ``storage and retrieval'' wording even before any ML features exist.
