"""MariaDB ColumnStore client helpers for analytics and inference storage."""
...
from .storage_config import columnstore_settings, parse_month
...
def get_engine(config: dict[str, Any] | None = None):
    """Return a SQLAlchemy engine for ColumnStore."""
...
def get_connection(config: dict[str, Any] | None = None) -> Iterator[Any]:
    """Yield a DBAPI connection from the SQLAlchemy engine."""
...
def ping(config: dict[str, Any] | None = None) -> bool:
    """Return True when ColumnStore accepts connections."""
...
def ensure_schema(config: dict[str, Any] | None = None) -> None:
    """No-op when schema is applied by Docker init; kept for programmatic use."""
    _ = config
    if not ping():
...
def bulk_upsert_month(
    ...
    *,
    ...
    """Replace rows for a year/month partition in a partitioned table."""
    with get_connection(config) as conn:
        cursor = conn.cursor()
        try:
...
def export_wide_parquet(
    ...
    *,
    ...
    """Export denormalized wide games for downstream preprocess."""
...
def insert_prediction_log(
    *,
    ...
    """Append one online inference record."""
    if not columnstore_enabled():
        return
...
def insert_batch_predictions(
    ...
    *,
    ...
    """Bulk insert offline evaluation predictions."""
    if df.empty:
        return 0
    ...
    if "y_true" in frame.columns and "target" not in frame.columns:
        frame["y_true"] = frame["y_true"]
    if "y_pred" in frame.columns and "prediction" not in frame.columns:
        frame["y_pred"] = frame["y_pred"]
...
def record_inference_run(
    *,
    ...
    """Record metadata for a batch inference/evaluation run."""
...
def fetch_batch_predictions_as_monitoring_frame(
    ...
    *,
    ...
    """Return batch predictions with target/prediction columns for Evidently."""
